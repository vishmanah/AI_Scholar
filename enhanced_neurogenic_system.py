"""
SISTEMA NEUROGENÉTICO AVANZADO CON MEJORAS CRÍTICAS
Versión mejorada con: memoria de largo plazo, atención, consolidación, y arquitectura optimizada
"""
...
class EpisodicMemory:
    ...
    def retrieve(self, query_embedding: torch.Tensor, top_k=5) -> List[KnowledgeNode]:
    ...
    def _consolidate(self):
        """Consolidación de memoria: eliminar menos importantes si hay sobrecarga"""
    ...
    def get_stats(self) -> Dict:
        return {
            ...
            'avg_access_count': np.mean([n.access_count for n in self.memories.values()]),
            'avg_importance': np.mean([n.importance_score for n in self.memories.values()])
        }
    ...
    def consolidate_knowledge(self):
        ...
        for i in range(len(self.gatekeeper_embeddings)):
            for j in range(i + 1, len(self.gatekeeper_embeddings)):
                ...
                if sim > 0.9:  # Muy similares
                    similarities.append((i, j, sim))
        
        # Fusionar módulos similares (implementación simplificada)
        # En producción, esto requeriría transferencia de pesos
...
class AdvancedKnowledgeExtractor:
    ...
    def get_knowledge_package(self, topic: str, depth: int = 1) -> Tuple[Optional[torch.Tensor], List[str], Dict]:
        ...
        try:
            ...
            # Extraer contenido
            text = page.summary
            links = list(page.links.keys())[:20]  # Limitar links
            
            # Procesar texto de manera más sofisticada
            sentences = [s.strip() for s in text.split('.') if len(s.strip()) > 20]
            
            if not sentences:
                return None, [], {}
            
            # Generar embeddings con contexto
            with torch.no_grad():
            ...
            # Embedding ponderado por posición (primeras oraciones más importantes)
...
class AdvancedAutonomousScholar:
    ...
    def __init__(self, initial_topic: str, logger_callback=print):
        ...
        self.priority_queue = []  # (priority_score, topic)
        
        # Mapeo de conocimiento
        self.module_map: Dict[int, List[str]] = {}
        self.knowledge_graph: Dict[str, List[str]] = {}
        
        # Métricas de meta-cognición
    ...
    def learn_one_step(self) -> bool:
        ...
        # Seleccionar siguiente tema (priorizado)
        ...
        # Expandir curiosidad de manera inteligente
    ...
    def _select_next_topic(self) -> str:
        """Seleccionar siguiente tema con priorización inteligente"""
    ...
    def _calculate_importance(self, metadata: Dict) -> float:
        """Calcular importancia de un concepto"""
        score = 0.5
    ...
    def _expand_curiosity(self, links: List[str], metadata: Dict):
        """Expansión inteligente de curiosidad"""
        # Priorizar links basado en términos clave
        ...
        # Ajustar curiosidad
        self.curiosity_score *= 0.99  # Decae levemente con el tiempo
...
if __name__ == "__main__":
    print("Sistema Neurogenético Avanzado - Versión Mejorada")
    print("=" * 60)
    
    # Inicializar
    scholar = AdvancedAutonomousScholar(initial_topic="Inteligencia Artificial")
